...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!store || !body.updateToken) return apiError(new Error("受付番号が無効です"), 403);
    const current = await getReceptionById(store, id);
    if (!current || current.updateToken !== body.updateToken) return apiError(new Error("受付番号が無効です"), 403);
